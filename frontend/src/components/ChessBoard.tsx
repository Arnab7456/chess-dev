          ...
          {row.map((square, j) => {
            ...
            return (
              <div
                ...
                key={j}
                className={`w-16 h-16 ${ (i + j) % 2 === 0 ? "bg-green-500" : "bg-white" }`} // Corrected the className template
              >
                <div className="w-full justify-center flex h-full">
                  <div className="h-full justify-center flex flex-col ">
                    {square ? (
                      <img
                        className={`w-10 ${square.color === "w" ? "filter invert" : ""}`} // Apply filter for white pieces
                        src={`/${square.type.toLowerCase()}.png`} // Use lowercase for the image file names
